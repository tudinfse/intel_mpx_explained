 ...
 *     (I build with `./entrypoint.py -v -d run -n micro_perf -t gcc_mpx -b multithreading_fn`)
 *     (I run with   `CHKP_RT_MODE=count CHKP_RT_PRINT_SUMMARY=1 CHKP_RT_VERBOSE=0 experiments/build/micro/perf/multithreading_fn/gcc_mpx/multithreading_fn`)
