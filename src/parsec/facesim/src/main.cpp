...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
	...
	parse_args.Add_String_Argument ("-data_dir", ".");
	parse_args.Parse (argc, argv);


    std::string data_dir;

	if (parse_args.Is_Value_Set ("-data_dir"))
	{
		data_dir = parse_args.Get_String_Value ("-data_dir");
	} else {
	    printf("Parsing error\n");
	    return 0;
	}
	STORYTELLING_EXAMPLE<float, float> example(data_dir);
