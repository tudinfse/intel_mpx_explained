...
//  struct thread_args chunk_thread_args[conf->nthreads];  \\ DMITRII KUVAISKII: MPX breaks Variable-Length Stack Arrays!
  struct thread_args chunk_thread_args[MAX_THREADS];
...
//  struct thread_args anchor_thread_args[conf->nthreads];  \\ DMITRII KUVAISKII: MPX breaks Variable-Length Stack Arrays!
  struct thread_args anchor_thread_args[MAX_THREADS];
  for (i = 0; i < conf->nthreads; i ++) {
...
//  struct thread_args compress_thread_args[conf->nthreads];  \\ DMITRII KUVAISKII: MPX breaks Variable-Length Stack Arrays!
  struct thread_args compress_thread_args[MAX_THREADS];
  for (i = 0; i < conf->nthreads; i ++) {
