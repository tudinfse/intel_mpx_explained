...
int close_file_thread( hnd_t handle )
{
   ...
   free( h->next_args );  h->next_args  = NULL;
   free( h );  h  = NULL;
    return 0;
}
#endif
...
int close_file_mp4( hnd_t handle )
{
    ...
    if (p_mp4->p_sample)
    {
        if (p_mp4->p_sample->data)
           free(p_mp4->p_sample->data); p_mp4->p_sample->data = NULL;
...
int close_file_mkv( hnd_t handle )
{
    ...
    if( p_mkv->sps )
       free( p_mkv->sps );  p_mkv->sps  = NULL;
    if( p_mkv->pps )
       free( p_mkv->pps );  p_mkv->pps  = NULL;
